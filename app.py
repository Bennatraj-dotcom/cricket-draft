...
# Sidebar Navigation
page = st.sidebar.radio("Go to", ["Admin Dashboard", "Selector Draft Room", "Team Rosters"], key="nav_menu_v3")
...
if page == "Admin Dashboard":
    ...
    if st.session_state.players:
        ...
        player_to_delete_str = st.selectbox("Select player profile to remove:", player_options, key="delete_select_v3")
        
        if st.button("Delete Player Profile", type="primary", key="del_btn_v3"):
...
# ------------------------------------------------------------------
    ...
    # Isolate players remaining in the active lot by comparing full dictionaries
    available_in_lot = []
    ...
    # Auto-advance lot if completely empty
    ...
    # Calculate selection position indexes (0, 1, 2, or 3)
    ...
    if players_picked_in_this_lot == 0:
        if st.button("🔀 Shuffle Pick Order for this Lot", type="secondary", key="shuffle_v3"):
            selectors = ["Selector 1", "Selector 2", "Selector 3", "Selector 4"]
            random.shuffle(selectors)
            st.session_state.draft_sequence = selectors
            st.rerun()
    ...
    # Visual Sequence Tracker Progress Grid
    ...
    if options:
        # Generate layout specific unique element tracking codes
        radio_id = f"radio_l{st.session_state.current_lot}_p{players_picked_in_this_lot}_len{len(options)}"
        selected_player_name = st.radio("Select the player called out by the captain:", options, key=radio_id)
        
        # FIXED AND ADDED ANTI-DOUBLE CLICK PROCESSING SAFEGUARD
        if st.button("Confirm Selection ➡️", type="primary", use_container_width=True, key=f"btn_{radio_id}"):
            # Enforce visual loading animation spinner layout block
            with st.spinner(f"Processing pick... Assigning to {current_turn_selector}"):
                ...
                # Lock data into session memory arrays
                st.session_state.teams[current_turn_selector].append(player_obj)
                
                # Small execution timeout delay padding (0.4s) to allow mobile processors to cycle rendering buffers cleanly
                time.sleep(0.4) 
                
            # Clear out layouts and move pointer forward instantly
            st.rerun()
    else:
